...
export const options = {
  ...
  thresholds: {
    ...
    scenario_request_duration: ['p(95)<3000', 'p(99)<6000'],
...
export function setup() {
  ...
  const response = http.post(
    `${BASE_URL}/performance/fixtures/same-course`,
    JSON.stringify({ capacity: CAPACITY }),
    jsonParams('perf_fixture_same'),
  );

  if (!check(response, { 'same-course fixture is ready': (result) => result.status === 200 })) {
    throw new Error(`same-course fixture failed: status=${response.status}`);
  }
  return { courseId: response.json('courses.0.id') };
}
...
export default function (data) {
  ...
  check(response, { 'same-course application succeeds': () => accepted });
  sleep(1);
}
